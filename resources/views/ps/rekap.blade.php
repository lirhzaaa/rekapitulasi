...
@section('content')
    ...
    <div class="btn-group" role="group" aria-label="Basic example">
        <a href="{{ route('latestsPs.home') }}" class="btn btn-outline-primary  ms-2">Keseluruhan Data</a>
        <a href="{{ route('latestsPs.rekap') }}" class="btn btn-primary  ms-2">Rekapitulasi Data</a>
    </div>
                    ...
                    @foreach ($result as $item => $value)
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    @if ($value >= 3)
                                        <form action="{{ route('latestsPs.download', $item) }}" method="">
                                            <button type="submit" class="btn btn-info text-white m-2"
                                                style="background: purple;">Export PDF</button>
                                        </form>
                                    @endif
                                    <form action="{{ route('latestsPs.detail', $item) }}" method="GEt">
